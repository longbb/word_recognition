...
if __name__ == '__main__':
                        ...
                        for syllable in syllables:
                            ...
                            if syllable in article.syllables_dictionary:
                                ...
                                if new_word:
                                    word_array.append('_'.join(new_word))
                                new_word = [syllable]
                                if state_sequence[index_state] == 2:
                                    index_state += 1
                        word_array.append('_'.join(new_word))
    ...
    new_word_text_file = module_path + '/result_data/hmm_by_me_new_word_all.txt'
    ...
    sentence_test = module_path + '/result_data/hmm_by_me_test_sentence_all.txt'
    ...
    # xa_hash = bigram_hash['xa']
    # most_appearances = []
    # for key, value in xa_hash.iteritems():
    #     try:
    #         if key == 'number_occurrences':
    #             continue
    #         if key == 'number_occurrences_in_bigram':
    #             continue
    #         if value['number_occurrences'] > 10:
    #             most_appearances.append({
    #                 key: value
    #             })
    #     except Exception as error:
    #         import pdb; pdb.set_trace()
    ...
    # test_sentence = 'đây là tiếng mẹ đẻ của khoảng 85% dân cư Việt Nam, cùng với hơn bốn triệu người Việt hải ngoại'.lower()
    # article = Article(test_sentence, 1, hmm_dictionary)
    # article_unlabeled_sequences = article.convert_syllable_to_number()
    # state_sequence = hmm.veterbi_algorithm(article_unlabeled_sequences[0])
    # word_array = []
    # syllables = test_sentence.split()
    # index_state = 0
    # new_word = []
    # for syllable in syllables:
    #     if syllable in article.syllables_dictionary:
